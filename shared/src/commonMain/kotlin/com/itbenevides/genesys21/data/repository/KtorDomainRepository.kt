...
class KtorDomainRepository(private val client: HttpClient) : DomainRepository {
    override suspend fun getAllMappings(): Result<List<DomainMapping>> =
        try {
    ...
    override suspend fun saveMapping(mapping: DomainMapping): Result<Unit> =
        try {
            val response =
                client.post("/api/admin/system/domains") {
            ...
            if (response.status.isSuccess()) {
                ...
                Result.failure(Exception("Erro ao salvar mapeamento"))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
    ...
    override suspend fun deleteMapping(id: String): Result<Unit> =
        try {
            ...
            if (response.status.isSuccess()) {
                ...
                Result.failure(Exception("Erro ao excluir mapeamento"))
            }
        } catch (e: Exception) {
            Result.failure(e)
        }
