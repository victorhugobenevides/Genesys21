...
class SqlitePageRepository : PageRepository {
    ...
    override suspend fun deletePage(id: String, token: String): Result<Unit> = try {
        dbQuery {
            ...
            if (deleted > 0) Result.success(Unit) else Result.failure(Exception("Página não encontrada ou sem permissão"))
        }
    } catch (e: Exception) {
        Result.failure(e)
    }
    ...
    override suspend fun uploadImage(bytes: ByteArray, fileName: String, token: String): Result<String> {
        return Result.failure(Exception("Operação não permitida via repositório"))
    }
