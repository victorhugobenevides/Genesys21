...
class SqliteReceiptRepository : ReceiptRepository {
    ...
    private fun ResultRow.toReceiptItem() =
        ReceiptItem(
    ...
    private fun ResultRow.toReceipt(items: List<ReceiptItem>) =
        Receipt(
    ...
    override suspend fun getAllReceipts(): Result<List<Receipt>> =
        try {
            dbQuery {
                ...
                val allReceipts =
                    allReceiptsRows.map { row ->
                        val items =
                            ReceiptItemsTable.selectAll().where { ReceiptItemsTable.receiptId eq row[ReceiptsTable.id] }
    ...
    suspend fun getReceiptsByStore(storeId: String): Result<List<Receipt>> =
        try {
            dbQuery {
    ...
    suspend fun getReceiptsByUser(userId: String): Result<List<Receipt>> =
        try {
            dbQuery {
    ...
    suspend fun saveReceiptWithUser(
        receipt: Receipt,
        userId: String,
    ): Result<Unit> =
        try {
